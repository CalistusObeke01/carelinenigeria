...
export class FaceScrubsService {
  url = `${ENV.BASE_API}/products`;
  constructor(private http: HttpClient) { }

  getFaceScrubs(catCode: string) {
  ...
  getFaceScrub(id: number): Observable<Organics> {
    if (id === 0) {
    ...
    return this.http.get<Organics>(url)
      .pipe(
        tap(data => console.log('FaceScrub: ' + JSON.stringify(data))),
        catchError(this.handleError)
      );
  }
