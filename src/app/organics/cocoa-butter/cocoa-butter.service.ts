...
export class CocoaButterService {
  url = `http://carelinenigeria.herokuapp.com/products`;
  constructor(private http: HttpClient) { }

  getCocoaButters(catCode: string) {
  ...
  getCocoaButter(id: number): Observable<Organics> {
    if (id === 0) {
    ...
    return this.http.get<Organics>(url)
      .pipe(
        tap(data => console.log(' getCocoaButter: ' + JSON.stringify(data))),
        catchError(this.handleError)
      );
  }
