...
export class SheaButtersService {
  url = `${ENV.BASE_API}/products`;
  constructor(private http: HttpClient) { }

  getSheaButters(catCode: string) {
  ...
  getSheaButter(id: number): Observable<Organics> {
    if (id === 0) {
    ...
    return this.http.get<Organics>(url)
      .pipe(
        tap(data => console.log('SheaButter: ' + JSON.stringify(data))),
        catchError(this.handleError)
      );
  }
